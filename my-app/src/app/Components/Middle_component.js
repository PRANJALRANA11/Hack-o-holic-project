...
export default function MiddleComponent() {
  ...
  useEffect(() => {
    const getUsers = async () => {
      try {
        const response = await users_api();
      ...
      <div className="" style={{ borderRadius: "4rem", paddingTop: "2rem", height: "30rem", marginTop: "3rem", width: "27rem" }}>
        ...
        {users.map((user) => (
          <div className="flex mt-6" key={user.id}>
            ...
            />
            <div className='w-24'>
            <p style={{ marginTop: "1.5rem", marginLeft: "1rem", fontWeight: "bold", fontSize: "1.5rem" }}>
              {user.name}
            </p>
            </div>
            <button
              type="button"
              className="text-white bg-blue-500 font-medium text-sm focus:outline-none"
              style={{ width: "10rem", borderRadius: "20rem", height: "3rem", marginTop: "1rem", marginLeft: "3rem" }}
            >
              Add friend
            </button>
          </div>
